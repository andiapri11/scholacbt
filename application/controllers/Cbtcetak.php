...
class Cbtcetak extends CI_Controller {
    ...
    public function getSiswaKelas() {
        ...
        $s = !$sesi ? null : $sesi;
        $ikelas = ($kelas == "all") ? $this->kelas->getIdKelas($tp->id_tp, $smt->id_smt) : $this->master->getKelasById($kelas);
        
        $isesi = ($s != null) ? $this->cbt->getSesiById($s) : null;
        $ijadwal = ($jadwal != null && $jadwal != "null") ? $this->cbt->getJadwalById($jadwal, $s) : null;
        ...
        $siswas = $this->cbt->getRuangSiswaByKelas($tp->id_tp, $smt->id_smt, ($kelas == "all" ? $ikelas : $kelas), $s);
    ...
    public function getSiswaRuang() {
        ...
        $pengawas = [];
        if ($pengawass != null && $pengawass->id_guru != '') {
            $pengawas = $this->master->getGuruByArrId(explode(",", $pengawass->id_guru));
        }
        
        $siswas = $this->cbt->getSiswaByRuang($tp->id_tp, $smt->id_smt, $ruang, $s);
    ...
    public function pengawas() {
        ...
        if ($jenis_selected != null) {
            $pengawas_raw = $this->cbt->getAllPengawas($tp->id_tp, $smt->id_smt);
            // Result is usually indexed by [id_jadwal][id_ruang][id_sesi]
            $pengawas = $pengawas_raw;
        }
        $data["pengawas"] = $pengawas;
        ...
        $jadwal_pengawas = [];
        foreach ($ruangs as $id_ruang => $ruang_data) {
            foreach ($ruang_data as $id_sesi => $sesi_data) {
                foreach ($kelas_level as $kl) {
                    foreach ($jadwals as $jadwal) {
        ...
        $result = [];
        $perRuang = [];
        foreach ($jadwal_pengawas as $tgl => $rooms) {
            foreach ($rooms as $r_id => $sessions) {
                foreach ($sessions as $s_id => $j_list) {
                    foreach ($j_list as $code => $km) {
                        $r_info = $ruangs[$r_id][$s_id];
                        $nr = $r_info->nama_ruang;
                        $ns = $r_info->nama_sesi;
                        $ir = $r_info->ruang_id;
                        $is = $r_info->sesi_id;
                        
                        $sel = isset($pengawas[$km->id_jadwal][$ir][$is]) ? explode(",", $pengawas[$km->id_jadwal][$ir][$is]->id_guru ?? '') : [];
                        $pw = "";
                        foreach ($sel as $idx => $p) {
                            if (isset($gurus[$p])) {
                                $pw .= $gurus[$p];
                                if ($idx < count($sel) - 1) $pw .= "<br>";
                            }
                        }
                        ...
                        $result[] = $forAdd;
                        if (!isset($perRuang[$nr])) $perRuang[$nr] = [];
                        $perRuang[$nr][] = $forAdd;
                    }
                }
            }
