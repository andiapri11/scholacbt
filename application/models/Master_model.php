...
class Master_model extends CI_Model {
    ...
    public function getAllEkstra() {
        return $this->db->get("master_extra")->result();
    }

    public function getEkstraById($id, $single = false) {
        if ($single === false) {
            $this->db->where_in("id_extra", $id);
            $this->db->order_by("nama_extra");
            $query = $this->db->get("master_extra")->result();
        } else {
            $query = $this->db->get_where("master_extra", array("id_extra" => $id))->row();
        }
        return $query;
    }

    function updateEkstra() {
        $id = $this->input->post("id_extra");
        $name = $this->input->post("nama_extra", true);
        $kode = $this->input->post("kode_extra", true);
        $this->db->set("nama_extra", $name);
        $this->db->set("kode_extra", $kode);
        $this->db->where("id_extra", $id);
        return $this->db->update("master_extra");
    }
