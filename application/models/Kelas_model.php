...
class Kelas_model extends CI_Model {
    ...
    public function get_one($id, $id_tp = null, $id_smt = null) {
        ...
        if ($id_tp != null) { $this->db->where('k.id_tp', $id_tp); }
        if ($id_smt != null) { $this->db->where('k.id_smt', $id_smt); }
    ...
    public function getJadwalMapelByMapel($kelas, $mapel, $tp, $smt) {
        ...
        if ($mapel != null) { $this->db->where('a.id_mapel', $mapel, FALSE); }
    ...
    public function getDummyJadwalMapel($tp, $smt, $jam, $kelas) {
        $inputData = [];
        for ($i = 1; $i < 7; $i++) {
            $inputData[] = json_decode(json_encode(['id_tp' => $tp, 'id_smt' => $smt, 'id_hari' => $i, 'jam_ke' => $jam, 'id_kelas' => $kelas, 'id_mapel' => '0', 'nama_mapel' => '', 'kode' => '']));
        }
        return $inputData;
    }
    ...
    public function getStatusMateriSiswa($id_kjm = null) {
        ...
        if ($id_kjm != null) { $this->db->where('a.id_materi', $id_kjm); }
        $result = $this->db->get()->result();
        $ret = [];
        if ($result) {
            foreach ($result as $row) {
    ...
    public function getMateriSiswa($id_kelas, $tgl, $jenis) {
        ...
        if ($result) {
            foreach ($result as $row) {
                ...
                $sisa = strlen($row->id_kjm) - ($len_kls + 10);
                $len = $sisa === 3 ? 2 : 1;
    ...
    public function getAllMateriByTgl($id_kelas, $tgl, $arr_mapel) {
        ...
        $this->db->where('a.id_kelas', $id_kelas);
        $this->db->where('a.jadwal_materi', $tgl);
        if (count($arr_mapel) > 0) { $this->db->where_in('a.id_mapel', $arr_mapel); }
        $result = $this->db->get()->result();
        $ret = [];
        if ($result) {
            foreach ($result as $row) {
    ...
    public function getRekapMateriSemester($id_kelas, $id_materi = null) {
        ...
        $this->db->from('log_materi');
        if ($id_materi != null) { $this->db->where('id_materi', $id_materi); }
        $result = $this->db->get()->result();
        $ret = [];
        if ($result) {
            foreach ($result as $row) {
                $len_kls = strlen($id_kelas);
                $len_tp_smt = 2; $len_tahun = 4; $len_bln = 2;
    ...
    public function getCatatanMapelBySiswa($id_kelas, $tp, $smt) {
        $this->db->select('a.*, b.nama_guru, b.foto, b.nip');
        ...
        $this->db->group_start();
        $this->db->where('a.id_kelas', $id_kelas);
        $this->db->or_where('a.type', '1');
        $this->db->group_end();
        return $this->db->get()->result();
    }

    public function getCatatanSiswaBySiswa($id_kelas, $tp, $smt) {
        $this->db->select('a.*, b.nama_guru, b.foto, b.nip');
        $this->db->from('kelas_catatan_wali a');
        $this->db->join('master_kelas d', 'a.id_kelas=d.id_kelas', 'left');
        $this->db->join('master_guru b', 'd.guru_id=b.id_guru', 'left');
        $this->db->where('a.id_tp', $tp);
        $this->db->where('a.id_smt', $smt);
        $this->db->group_start();
        $this->db->where('a.id_kelas', $id_kelas);
    ...
    public function getCatatanMapelKelas($id_kelas, $id_mapel, $tp, $smt) {
        $this->db->select('*');
    ...
    public function getCatatanMapelSiswa($tp, $smt, $id_kelas, $id_mapel) {
        $this->db->select('a.*, b.nama as nama_siswa');
        $this->db->from('kelas_catatan_mapel a');
        $this->db->join('master_siswa b', 'a.id_siswa=b.id_siswa');
        $this->db->where('a.id_tp', $tp);
        $this->db->where('a.id_smt', $smt);
    ...
    public function getAllCatatanMapelSiswa($id_siswa, $id_mapel, $tp, $smt) {
        $this->db->select('*');
        $this->db->from('kelas_catatan_mapel');
        $this->db->where('id_siswa', $id_siswa);
        $this->db->where('id_mapel', $id_mapel);
    ...
    public function getCatatanSiswa($tp, $smt, $id_kelas) {
        $this->db->select('a.*, b.nama as nama_siswa');
        $this->db->from('kelas_catatan_wali a');
        $this->db->join('master_siswa b', 'a.id_siswa=b.id_siswa');
        $this->db->where('a.id_tp', $tp);
    ...
    public function getCatatanKelas($id_kelas, $tp, $smt) {
        $this->db->select('*');
    ...
    public function getAllCatatanSiswa($id_siswa, $tp, $smt) {
        $this->db->select('*');
        $this->db->from('kelas_catatan_wali');
        $this->db->where('id_siswa', $id_siswa);
        $this->db->where('id_tp', $tp);
    ...
    public function getCatatanMapelSiswaDetail($id_catatan) {
        $this->db->select('a.*, b.nama_guru, b.foto, c.nama_mapel');
        $this->db->from('kelas_catatan_mapel a');
        $this->db->join('master_guru b', 'a.id_guru=b.id_guru');
        $this->db->join('master_mapel c', 'a.id_mapel=c.id_mapel');
        $this->db->where('a.id_catatan', $id_catatan);
        return $this->db->get()->row();
    }

    public function getCatatanKelasSiswaDetail($id_catatan) {
        $this->db->select('a.*, b.nama_guru, b.foto, d.nama_kelas, "Wali Kelas" as jabatan');
        $this->db->from('kelas_catatan_wali a');
        $this->db->join('master_kelas d', 'a.id_kelas=d.id_kelas', 'left');
        $this->db->join('master_guru b', 'd.guru_id=b.id_guru', 'left');
        $this->db->where('a.id_catatan', $id_catatan);
